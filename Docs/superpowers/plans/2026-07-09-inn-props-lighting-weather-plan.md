...
Replace the first-floor inn's primitive prop placeholders with a fully dressed hybrid prop kit, add an animated courtyard fountain, prepare reusable upstairs bed/rest behavior, and make windows and interior lighting reflect persistent world time and weather without changing the approved layout.
...
Verification: a written asset/runtime inventory exists, integration points are identified, and Unity reports zero baseline errors.
...
## Phase 7: Windows and Persistent Time
...
3. Implement an inn time-of-day adapter that reads—but does not advance—the persistent world clock.
4. Drive courtyard sun direction, intensity, color, shadows, ambient light, window exterior presentation, and interior lamp intensity from that state.
5. Tune dawn, noon, sunset, and night profiles for readability and continuity with outdoor maps.
6. Confirm scene entry and exit preserve the current hour.

Verification: forced dawn/noon/sunset/night states produce distinct correct visuals, and the state remains unchanged across scene transitions.

## Phase 8: Persistent Weather Through Windows

1. Implement a window-weather adapter using the global weather state and events.
...
4. Synchronize storm lightning window flashes and temporary interior illumination to the outdoor lightning event/state.
...
Verification: weather changes update without scene reload; lightning flashes remain synchronized; entering/leaving preserves weather; no indoor precipitation appears.
...
5. Instantiate fountain, windows, time adapter, weather adapter, and their references through the builder.
...
Verification: repeat builds are deterministic, no placeholders return, and all references resolve.
...
9. Capture final top, isometric, gameplay-camera, and time/weather comparison images.
10. Confirm zero Unity console errors and warnings after a complete playthrough.
...
4. Commit only the prop kit, approved generated/imported assets, scripts, builder changes, rebuilt scene, and required configuration data.
