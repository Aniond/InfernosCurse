...
public static class FlorenceLimboWorldAuthoringBuilder
{
    ...
    public static void Rebuild()
    {
        ...
        Debug.Log("[FlorenceLimboWorldAuthoring] Rebuilt 1 Crier, 5 NPC memory definitions, 5 discoveries, and 4 Mercato sites.");
    }
    ...
    static void BuildNpcDefinitions()
    {
        BuildNpc(
            ...
            new[] { PreachingSiteIds[0], PreachingSiteIds[2] }, false, false);
        BuildNpc(
            "Npc_Mercato_Baker.asset", "npc_mercato_baker", 1.00f,
            "schedule_mercato_baker", "relationship_mercato_usual_customer",
            new[] { PreachingSiteIds[0], PreachingSiteIds[1] }, false, false);
        BuildNpc(
    ...
    static void BuildMercatoSites()
    {
        ...
        CreateSite(
            ...
            AnchorPosition(scene, "Fountain", new Vector3(3f, 0f, 2f), new Vector3(0f, 0f, 0f)));
        CreateSite(
            root.transform, PreachingSiteIds[1], WorldAgentSiteRole.Preach,
            AnchorPosition(scene, "[Florentine_Stall_1]", new Vector3(2f, 0f, -1f), new Vector3(-12f, 0f, 4f)));
        CreateSite(
            root.transform, PreachingSiteIds[2], WorldAgentSiteRole.Preach,
            AnchorPosition(scene, "EntryPoint_South", new Vector3(4f, 0f, 5f), new Vector3(0f, 0f, -14f)));
        CreateSite(
            root.transform, "mercato_crier_hide_barrel_alley", WorldAgentSiteRole.Hide,
            AnchorPosition(scene, "GapFill_Barrel_7", new Vector3(1f, 0f, 1f), new Vector3(-20f, 0f, 10f)));
...
public static class FlorenceLimboWorldAuthoringValidator
{
    ...
    public static void Validate()
    {
        ...
        Debug.Log("[FlorenceLimboWorldAuthoringValidator] Validation passed: 1 persistent Crier, 5 safeguarded NPC records, 5 hidden discoveries, and 4 authored Mercato sites.");
    }
    ...
    static void ValidateAgent(List<string> errors)
    {
        ...
        if (definition.agentId != "limbo_crier_mercato_01" || definition.districtId != "mercato" ||
    ...
    static void ValidateNpcs(List<string> errors)
    {
        ...
        foreach (string guid in AssetDatabase.FindAssets("t:NpcMemoryDefinition", new[] { "Assets/Resources/NpcMemory" }))
        {
            ...
            if (definition.homeDistrictId != "mercato" || definition.susceptibility < 0.5f ||
